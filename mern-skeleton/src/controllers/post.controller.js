...
const postByID = async (req, res, next, id) => {
    try {
        let post = await Post.findById(id).populate("postedeBy", "_id name").exec()
        if (!post) {
            return res.status("400").json({
...
export default { listNewsFeed, create, photo, postByID, isPoster, remove, like, unlike, comment,uncomment }